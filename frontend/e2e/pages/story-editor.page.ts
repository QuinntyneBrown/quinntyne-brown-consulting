...
export class StoryEditorPage {
  ...
  async createStory(title: string, epicName: string, assistantName: string, taskTitle: string): Promise<void> {
    ...
    await dialog.getByLabel('Description or user story').fill('As a consultant, I want a verified delivery workflow so that work remains clear.');
    await dialog.getByLabel('Acceptance criteria').fill('The workflow persists and can be completed.');
  ...
  async archiveOpenStory(): Promise<void> {
    ...
    await this.page.getByRole('dialog', { name: /Archive QBC-/ }).getByRole('button', { name: 'Archive' }).click();
    await expect(dialog).toBeHidden();
  }
  ...
  private openDialog() {
    return this.page.getByRole('dialog').filter({ has: this.page.getByRole('button', { name: 'Save story' }) });
  }
}
