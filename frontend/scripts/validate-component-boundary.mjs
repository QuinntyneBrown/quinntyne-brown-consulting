...
const componentFiles = filesUnder(join(componentSource, 'lib')).filter(file => file.endsWith('.component.ts'));
...
if (manifest.version !== packageMetadata.version) {
  failures.push(`Component manifest version ${manifest.version} does not match package version ${packageMetadata.version}.`);
}
if (JSON.stringify(expectedCatalog) !== JSON.stringify(declaredCatalog)) {
  failures.push('Angular manifest catalogComponents does not match design-system/component-manifest.json.');
}
...
for (const [selector, file] of selectorFiles) {
  const exportPath = './' + relative(componentSource, file).split(sep).join('/').replace(/\.ts$/, '');
  if (!publicApi.includes(`'${exportPath}'`)) failures.push(`${selector} is not exported from the public API.`);
}

const appFiles = filesUnder(appSource);
const forbiddenElement = /<(button|input|select|textarea|dialog|a)(?:\s|>)/gi;
const legacyClass = /\b(primary-button|secondary-button|quiet-button|danger-button|icon-button|dialog-shell|dialog-head|dialog-body|dialog-actions)\b/g;
const legacyToken = /var\(--(ink|muted|line|soft|panel|accent|accent-dark|accent-soft|danger|danger-soft|radius|shadow)\b/g;
...
for (const file of appFiles) {
  ...
  if (extension === '.html') {
    ...
    if (native.length) failures.push(`${display} contains native UI elements: ${[...new Set(native)].join(', ')}.`);
    const classes = [...source.matchAll(legacyClass)].map(match => match[1]);
    if (classes.length) failures.push(`${display} contains legacy component classes: ${[...new Set(classes)].join(', ')}.`);
  }
  if (extension === '.scss' && legacyToken.test(source)) failures.push(`${display} consumes legacy design tokens.`);
...
for (const file of filesUnder(componentSource)) {
  ...
  if (source.includes('@qbc/api') || source.includes('qbc-workboard')) {
    failures.push(`${relative(workspace, file)} couples @qbc/components to application or API code.`);
  }
}
...
if (failures.length) {
  ...
  console.log(`Component boundary valid: ${selectorFiles.size} exported Angular components, including all ${expectedCatalog.length} catalog components.`);
}
