...
- **`HierarchyPageComponent`** — Angular route component that renders initiative
  cards, epic rows, counts, progress, and contextual creation actions. Creating
  and updating a record opens its own editor route rather than a form here,
  because a name and a markdown document are saved together; see
  [Edit the initiative brief](../edit-initiative-brief/). The page holds no form
  at all now, only the confirmations that guard a deletion.
- **`summariseMarkdown`** — reduces a markdown document to its first line of
  prose, which is what a card or a row has room to carry. The hierarchy uses it
  for both an initiative's brief and an epic's summary.
- An epic's create, update, and parent movement open its editor route for the
  same reason an initiative's do; see
  [Write an epic summary](../edit-epic-summary/).
