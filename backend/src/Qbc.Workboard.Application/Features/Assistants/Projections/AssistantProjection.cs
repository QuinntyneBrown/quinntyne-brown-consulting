...
{
    public static AssistantDto Create(
        Assistant assistant,
        ...
        IReadOnlyList<TimeEntry> timeEntries)
    {
        ...
        // Hours the assistant logged block the delete too: they record who did the work, and a
        // story they no longer own can still be holding their time.
        var loggedAgainst = timeEntries.Where(entry => entry.AssistantId == assistant.Id)
            .Select(entry => entry.StoryId)
            .Distinct()
            .Where(storyId => ownedStories.All(story => story.Id != storyId))
            .Select(storyId => stories.Single(item => item.Id == storyId));
        var links = ownedStories.Concat(loggedAgainst)
            .Select(story => new AssignmentLinkDto(story.Id, story.Key, null, story.Title))
